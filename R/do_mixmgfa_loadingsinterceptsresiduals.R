#' @export
do_mixmgfa_loadingsinterceptsresiduals <- function(data2,N_gs,nclust,nfactors,varlabels,grouplabels,maxiter = 5000,start = 1,nruns = 25,design=design,rotation=0,preselect=10,targetT=0,targetW=0,parcomp=0){
  ...
  output_nclust2<-list(clustermemberships=output_nclust$z_gks,clusterproportions=output_nclust$pi_ks,clusterspecific.loadings=output_nclust$Lambda_ks,group.and.clusterspecific.factorcovariances=output_nclust$Phi_gks,clusterspecific.uniquevariances=output_nclust$Psi_ks,clusterspecific.intercepts=output_nclust$tau_ks,group.and.clusterspecific.factormeans=output_nclust$alpha_gks,loglik=output_nclust$bestloglik,nrpars=output_nclust$nrpars,convergence=output_nclust$convergence>0,nractivatedconstraints=output_nclust$nractivatedconstraints)
