#' @export
do_mixmgfa_residuals <- function(data2,N_gs,nclust,nfactors,varlabels,grouplabels,maxiter = 5000,start = 1,nruns = 25,design=design,rotation=0,preselect=10,targetT=0,targetW=0,parcomp=0){
  ...
  output_nclust2<-list(clustermemberships=output_nclust$z_gks,clusterproportions=output_nclust$pi_ks,invariant.loadings=output_nclust$Lambda,groupspecific.factorcovariances=output_nclust$Phi_gs,clusterspecific.uniquevariances=output_nclust$Psi_ks,invariant.intercepts=output_nclust$tau,groupspecific.factormeans=output_nclust$alpha_gs,loglik=output_nclust$bestloglik,nrpars=output_nclust$nrpars,convergence=output_nclust$convergence>0,nractivatedconstraints=output_nclust$nractivatedconstraints)
