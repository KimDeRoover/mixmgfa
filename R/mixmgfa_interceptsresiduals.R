...
mixmgfa_interceptsresiduals <- function(data,N_gs,nclust,nfactors=1,maxiter = 5000,start = 1,nruns = 25,design = 0,preselect = 10,startpartition){
        ...
        for (trialstart in 1:nrtrialstarts){ # select 'preselect'% (default 10%) best fitting random partitions
          ...
          for(g in 1:ngroup){
            ...
            for(k in 1:nclust){
              ...
              S_gk=S_gks[[g,k]]
              theta_gk=phi_g-beta_gk%*%(Lambda%*%phi_g-S_gk%*%tbeta_gk)
              Theta_gks[[g,k]]=theta_gk
              meanexpEta_gks[[g,k]]=(mean_gs[g,]-tau_ks[k,]-alpha_gks[[g,k]]%*%tLambda)%*%tbeta_gk
            }
          }
  ...
  for(k in 1:nclust){
    ...
    if(N_ks[k]>0){
      ...
      for(g in 1:ngroup){
        invSigma_gk=invSigma_gks[[g,k]]
        summeansminusalphaLambdainvSigma=summeansminusalphaLambdainvSigma+N_gks[g,k]*(mean_gs[g,]-alpha_gks[[g,k]]%*%tLambda)%*%invSigma_gk
    ...
    if(N_ks[k]>0){
      ...
      for(g in 1:ngroup){
        if(N_gks[g,k]>0){
          ...
          beta_gk=Beta_gks[[g,k]]
          theta_gk=Theta_gks[[g,k]]
          sumS_2SbetaB_BthetaB=sumS_2SbetaB_BthetaB+(N_gks[g,k]/N_ks[k])*(S_gk-Lambda%*%(2*beta_gk%*%S_gk-theta_gk%*%tLambda))
        }
      }
