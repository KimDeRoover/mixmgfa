      ...
      for (trialstart in 1:nrtrialstarts){ # select 'preselect'% (default 10%) best fitting random partitions
        ...
        for(k in 1:nclust){
          ...
          for(g in 1:ngroup){
            ...
            invPhi_g_tLambdainvPsi_k_Lambda=(invPhi_gk+tlambda_k%*%invPsi_k%*%lambda_k)
            invPhi_g_tLambdainvPsi_k_Lambda=(invPhi_g_tLambdainvPsi_k_Lambda+t(invPhi_g_tLambdainvPsi_k_Lambda))*(1/2)
            invSigma_gks[[g,k]]=invPsi_k-invPsi_k%*%lambda_k%*%solve(invPhi_g_tLambdainvPsi_k_Lambda)%*%tlambda_k%*%invPsi_k # Woodbury identity
          }
        }
        ...
        for(g in 1:ngroup){
          for(k in 1:nclust){
            ...
            S_gk=CP_gs[[g]]-t(mean_g)%*%mu_gk+t(mu_gk)%*%(mu_gk-mean_g)#CP_gs[[g]]-t(mean_g)%*%mu_gk-t(mu_gk)%*%mean_g+t(mu_gk)%*%mu_gk
            S_gks[[g,k]] <- S_gk
          }
        }
        ...
        for(g in 1:ngroup){
          for(k in 1:nclust){
            ...
            meanexpEta_gks[[g,k]]=(mean_gs[g,]-tau_ks[k,]-alpha_gks[[g,k]]%*%tlambda_k)%*%t(beta_gk)
          }
        }
